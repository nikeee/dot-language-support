...
import { SourceFile, SyntaxNodeFlags, SyntaxKind, Assignment, CharacterCodes, NodeId } from "../types";
...
interface StaticCompletions {
	[i: string]: StaticCompletion
}
interface StaticCompletion {
	kind: lst.CompletionItemKind;
	values: string[]
}

const staticCompletions: StaticCompletions = {
	"shape": {
		kind: lst.CompletionItemKind.EnumMember,
		values: [
			"box",
			"polygon",
			"ellipse",
	...
	"color": {
		kind: lst.CompletionItemKind.Color,
		values: [
			"aliceblue",
			"antiquewhite",
			"antiquewhite1",
